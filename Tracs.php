<?php
    ...
    class Tracs implements Systems {
			...
			if ($this->is_session_id == 'true') {
				...
				for ($i = 0; $i < $num_of_tables; $i++) {
					if ($i == 0) {
						...
						$result = $this->DoesEmailExistContact();
                        // Table name
						$table_name = 'Contacts';
					}
		...
		Private function DoesEmailExistContact() {
			//Spoof email;
			$email = 'mail@mail.com';
		    // SQL statement with placeholder
			$sql = 'SELECT TRACS.GDPR_SEARCH.SEARCH_CONTACT(:email) FROM DUAL';
